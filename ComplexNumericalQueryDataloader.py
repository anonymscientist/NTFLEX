...
from ComplexNumericalQueryData import TYPE_train_queries_answers, TYPE_test_queries_answers
from NTFLEX_DSL import is_to_predict_entity_set, is_to_predict_timestamp_set, query_structures
...
class TrainDataset(Dataset):
    def __init__(self, queries_answers: TYPE_train_queries_answers, entity_count: int, value_count: int, timestamps_count: int, negative_sample_size: int):
...
def flatten_test2(queries_answers: TYPE_test_queries_answers) -> List[Tuple[str, List[int], Set[int]]]:
    res = []
    for query_name, query_schema in queries_answers.items():
        qa_list: List[Tuple[List[int], Set[int], Set[int]]] = query_schema["queries_answers"]
        for query, easy_answer, hard_answer in qa_list:
            res.append((query_name, query, hard_answer))
    return res


class TestDataset(Dataset):
    def __init__(self, queries_answers: TYPE_test_queries_answers, entity_count: int, value_count: int, timestamps_count: int):
    ...
    def __getitem__(self, idx):
        ...
        hard_answer_mask = torch.zeros(answer_range).bool()
        if len(hard_answer) > 0:
            hard_answer_mask[list(hard_answer)] = True
        return query_name, query, candidate_answer, easy_answer_mask, hard_answer, hard_answer_mask
    ...
    def collate_fn(data):
        ...
        grouped_hard_answer_mask_dict: Dict[str, List[torch.Tensor]] = defaultdict(list)
        for i, (query_name, query, candidate_answer, easy_answer_mask, hard_answer, hard_answer_mask) in enumerate(data):
            if None in query:
            ...
            grouped_easy_answer_dict[query_name].append(easy_answer_mask)
            grouped_hard_answer_dict[query_name].append(hard_answer)
            grouped_hard_answer_mask_dict[query_name].append(hard_answer_mask)
            ...
            if key_DM in query_structures:
                ...
                grouped_easy_answer_dict[key_DM].append(easy_answer_mask)
                grouped_hard_answer_dict[key_DM].append(hard_answer)
                grouped_hard_answer_mask_dict[key_DM].append(hard_answer_mask)
        ...
        grouped_hard_answer_mask: Dict[str, torch.Tensor] = {
            key: torch.stack(grouped_hard_answer_mask_dict[key], dim=0)
            for key in grouped_hard_answer_mask_dict
        }
        return grouped_query, grouped_candidate_answer, grouped_easy_answer, grouped_hard_answer_dict, grouped_hard_answer_mask
    
    @staticmethod
    def collate_fn_rel(data):
        batch_queries_idx_dict: Dict[str, List[List[int]]] = defaultdict(dict)
        batch_candidate_answer_dict: Dict[str, List[torch.Tensor]] = defaultdict(dict)
        grouped_easy_answer_dict: Dict[str, List[torch.Tensor]] = defaultdict(dict)
        grouped_hard_answer_dict: Dict[str, List[Set[int]]] = defaultdict(dict)
        grouped_hard_answer_mask_dict: Dict[str, List[torch.Tensor]] = defaultdict(dict)
        for i, (query_name, query, candidate_answer, easy_answer_mask, hard_answer, hard_answer_mask) in enumerate(data):
            if None in query:
                print("error", query_name, query)
            try:
                batch_queries_idx_dict[query_name][query[1]].append(query)
            except KeyError:
                batch_queries_idx_dict[query_name].update({query[1]: [query]})
            try:
                batch_candidate_answer_dict[query_name][query[1]].append(candidate_answer)
            except KeyError:
                batch_candidate_answer_dict[query_name].update({query[1]: [candidate_answer]})
            try:
                grouped_easy_answer_dict[query_name][query[1]].append(easy_answer_mask)
            except KeyError:
                grouped_easy_answer_dict[query_name].update({query[1]: [easy_answer_mask]})
            try:
                grouped_hard_answer_dict[query_name][query[1]].append(hard_answer)
            except KeyError:
                grouped_hard_answer_dict[query_name].update({query[1]: [hard_answer]})
            try:
                grouped_hard_answer_mask_dict[query_name][query[1]].append(hard_answer_mask)
            except KeyError:
                grouped_hard_answer_mask_dict[query_name].update({query[1]: [hard_answer_mask]})

            # in FLEX, it has used DNF for union
            # here we only cope with DM
            key_DM = f"{query_name}_DM"
            if key_DM in query_structures:
                batch_queries_idx_dict[key_DM][query[1]].append(query)
                batch_candidate_answer_dict[key_DM][query[1]].append(candidate_answer)
                grouped_easy_answer_dict[key_DM][query[1]].append(easy_answer_mask)
                grouped_hard_answer_dict[key_DM][query[1]].append(hard_answer)
                grouped_hard_answer_mask_dict[key_DM][query[1]].append(hard_answer_mask)

        grouped_query: Dict[str, Dict[int, torch.Tensor]] = defaultdict(dict)
        for query in batch_queries_idx_dict:
            for rel in batch_queries_idx_dict[query]:
                grouped_query[query].update({rel: torch.LongTensor(batch_queries_idx_dict[query][rel])})

        grouped_candidate_answer: Dict[str, Dict[int, torch.Tensor]] = defaultdict(dict)
        for query in batch_candidate_answer_dict:
            for rel in batch_candidate_answer_dict[query]:
                grouped_candidate_answer[query].update({rel: torch.stack(batch_candidate_answer_dict[query][rel], dim=0)})

        grouped_easy_answer: Dict[str, Dict[int, torch.Tensor]] = defaultdict(dict)
        for query in grouped_easy_answer_dict:
            for rel in grouped_easy_answer_dict[query]:
                grouped_easy_answer[query].update({rel: torch.stack(grouped_easy_answer_dict[query][rel], dim=0)})

        grouped_hard_answer_mask: Dict[str, Dict[int, torch.Tensor]] = defaultdict(dict)
        for query in grouped_hard_answer_mask_dict:
            for rel in grouped_hard_answer_mask_dict[query]:
                grouped_hard_answer_mask[query].update({rel: torch.stack(grouped_hard_answer_mask_dict[query][rel], dim=0)})
...
class TestDataset2(Dataset):
    def __init__(self, queries_answers: TYPE_test_queries_answers, entity_count: int, value_count: int, timestamps_count: int, negative_sample_size: int):
        self.all_data: List[Tuple[str, List[int], Set[int]]] = flatten_test2(queries_answers)
        self.len: int = len(self.all_data)
        self.entity_count: int = entity_count
        self.value_count: int = value_count
        self.timestamps_count: int = timestamps_count
