...
import expression
from ComplexTemporalQueryData import ICEWS05_15, ICEWS14, GDELT, WIKI, ComplexTemporalQueryDatasetCachePath, TemporalComplexQueryData, TYPE_train_queries_answers, groups
...
class MyExperiment(Experiment):
    ...
    def __init__(self, output: OutputSchema, data: TemporalComplexQueryData, model,
        ...
        if eval_all:
            data.load_cache(["valid_queries_answers"])
            data.load_cache(["test_queries_answers"])
        else:
...
@click.option("--dataset", type=str, default="WIKI", help="Which dataset to use: ICEWS14, ICEWS05_15, GDELT.")
...
@click.option('--train_tasks', type=str, default="Pe,Pe2,e2i,"
              + "Pt,aPt,bPt,t2i", help='the tasks for training')
@click.option('--train_all', type=bool, default=False,
              help='if training all, it will use all tasks in data.train_queries_answers')
@click.option('--eval_tasks', type=str, default="Pe,Pe2,e2i,"
              + "Pt,aPt,bPt,t2i," +
              "e2u,t2u", help='the tasks for evaluation')
