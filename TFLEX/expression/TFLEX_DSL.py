...
class SamplingParser(BasicParser):
    def __init__(self, entity_ids: List[int], relation_ids: List[int], timestamp_ids: List[int],
        ...
        def fast_e2i_Pe(e1, r1, t1, r2, t2, e2, r3, t3):
            # return And(Pe(Pe(e1, r1, t1), r2, t2), Pe(e2, r3, t3))
            o_idx = random.choice(valid_e2i_o_list)
            right_o_ids = fast_Pe_targeted(e2, r3, t3, target=o_idx)
            left_o_ids = fast_Pe2_targeted(e1, r1, t1, r2, t2, target=o_idx)
            return EntitySet(left_o_ids & right_o_ids)
        
        def fast_e3i_N(e1, r1, t1, e2, r2, t2, e3, r3, t3): 
            # return And3(Pe(e1, r1, t1), Pe(e2, r2, t2), Not(Pe(e3, r3, t3)))
            o = random.choice(valid_e3i_o_list)
            choices = set(all_entity_ids - {o}) & set(valid_e3i_o_list)
            while len(choices) <= 0:
                o = random.choice(valid_e3i_o_list)
                choices = set(all_entity_ids - {o}) & set(valid_e3i_o_list)
            not_o = random.choice(list(choices))
            left_o_ids = fast_Pe_targeted(e1, r1, t1, target=o)
            middle_o_ids = fast_Pe_targeted(e2, r2, t2, target=o)
            right_o_ids = all_entity_ids - fast_Pe_targeted(e3, r3, t3, target=not_o)
            return EntitySet(left_o_ids & middle_o_ids & right_o_ids)

        def fast_t3i_N(e1, r1, e2, e3, r2, e4, e5, r3, e6): 
            # return TimeAnd3(Pt(e1, r1, e2), Pt(e3, r2, e4), TimeNot(Pt(e5, r3, e6)))
            o = random.choice(valid_t3i_t_list)
            choices = set(all_timestamp_ids - {o}) & set(valid_t3i_t_list)
            while len(choices) <= 0:
                o = random.choice(valid_t3i_t_list)
                choices = set(all_timestamp_ids - {o}) & set(valid_t3i_t_list)
            not_o = random.choice(list(choices))
            left_o_ids = fast_Pt_targeted(e1, r1, e2, target=o)
            middle_o_ids = fast_Pt_targeted(e3, r2, e4, target=o)
            right_o_ids = all_timestamp_ids - fast_Pt_targeted(e5, r3, e6, target=not_o)
            return EntitySet(left_o_ids & middle_o_ids & right_o_ids)
        ...
        self.fast_ops = {
            ...
            "fast_e2i_Pe": fast_e2i_Pe,
            "fast_e3i_N": fast_e3i_N,
            "fast_t3i_N": fast_t3i_N
        }
