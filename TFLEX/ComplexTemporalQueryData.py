...
from assistance.toolbox.data.DataSchema import DatasetCachePath, BaseData
from assistance.toolbox.data.DatasetSchema import RelationalTripletDatasetSchema
from assistance.toolbox.data.functional import read_cache, cache_data
...
class TemporalComplexQueryData(TemporalKnowledgeData):
    ...
    def sampling(self):
        ...
        if "Pe" not in self.train_queries_answers:
            ...
            cache_data(self.train_queries_answers["Pe"], self.cache_path.cache_queries_answers_path("train", "Pe"))
        if "Pe" not in self.valid_queries_answers:
            self.valid_queries_answers["Pe"] = build_one_hop(["e1", "r1", "t1"], valid_srt_o, for_test=True)
            cache_data(self.valid_queries_answers["Pe"], self.cache_path.cache_queries_answers_path("valid", "Pe"))
        if "Pe" not in self.test_queries_answers:
            self.test_queries_answers["Pe"] = build_one_hop(["e1", "r1", "t1"], test_srt_o, for_test=True)
            cache_data(self.test_queries_answers["Pe"], self.cache_path.cache_queries_answers_path("test", "Pe"))
        print("Pe",
        ...
        if "Pt" not in self.train_queries_answers:
            ...
            cache_data(self.train_queries_answers["Pt"], self.cache_path.cache_queries_answers_path("train", "Pt"))
        if "Pt" not in self.valid_queries_answers:
            self.valid_queries_answers["Pt"] = build_one_hop(["e1", "r1", "e2"], valid_sro_t, for_test=True)
            cache_data(self.valid_queries_answers["Pt"], self.cache_path.cache_queries_answers_path("valid", "Pt"))
        if "Pt" not in self.test_queries_answers:
            self.test_queries_answers["Pt"] = build_one_hop(["e1", "r1", "e2"], test_sro_t, for_test=True)
            cache_data(self.test_queries_answers["Pt"], self.cache_path.cache_queries_answers_path("test", "Pt"))
        print("Pt",
        ...
        train_sample_counts = {
            ...
            "Pt" : max_sample_count,  
            "aPt" : max_sample_count, 
            "bPt" : max_sample_count, 
            "t2i" : max_sample_count,
        }
        test_sample_count = 5000
        test_sample_counts = {
            "Pe" : test_sample_count,
            "Pe2" : test_sample_count,
            "e2i" : test_sample_count,

            "Pt" : test_sample_count,  
