...
int relationTotal, entityTotal, tripleTotal, valueTotal;
int *freqRel, *freqEnt;
float *left_mean, *right_mean;
float *valueList;

AttrTriple *attrList;
int attrTotal, attpTotal;
...
void init() {
    ...
    fclose(fin);
	printf("value2id abgeschlossen\n");
	fin = fopen((inPath + "attrm2id.txt").c_str(), "r");
	tmp = fscanf(fin, "%d", &attpTotal);
	attrList = (AttrTriple *)calloc(attpTotal, sizeof(AttrTriple));
	attpTotal = 0;
	while (fscanf(fin, "%d", &attrList[attpTotal].e) == 1) {
		tmp = fscanf(fin, "%d", &attrList[attpTotal].a);
		tmp = fscanf(fin, "%d", &attrList[attpTotal].v);
		attpTotal++;
	}
	fclose(fin);
	printf("attrm2id abgeschlossen\n");
	fin = fopen((inPath + "relation2id.txt").c_str(), "r");
	tmp = fscanf(fin, "%d", &relationTotal);
	fclose(fin);
	printf("relation2id abgeschlossen\n");
	freqRel = (int *)calloc(relationTotal, sizeof(int));
	
	fin = fopen((inPath + "entity2id.txt").c_str(), "r");
	tmp = fscanf(fin, "%d", &entityTotal);
	fclose(fin);
	printf("entity2id abgeschlossen\n");
	...
	fclose(fin);
	printf("triple2id abgeschlossen\n");
...
extern "C"
void getAttrBatch(int *pe, int *pa, float *pv, int batchSize, int id = 0) {
	for (int batch = 0; batch < batchSize; batch++) {
		int i = rand_max(id, attpTotal);
		pe[batch] = attrList[i].e;
		pa[batch] = attrList[i].a;
		pv[batch] = valueList[attrList[i].v];
	}
}
